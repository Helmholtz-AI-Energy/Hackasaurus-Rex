...
# and `pip install hackasaurus-rex` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "hackasaurus-rex", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n hackasaurus-rex -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "hackasaurus-rex", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "hackasaurus-rex", "YOUR_CONSOLE_SCRIPT"]
